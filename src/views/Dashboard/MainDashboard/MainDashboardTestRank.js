...
class MainDashboardTestRank extends React.Component{
    constructor(props) {
        super(props);
        this.state = { items:[], index: 0 ,subject:"" ,  it: true , open: false , fetch: false , scrolluser : "" , showprofile: false , username : "" , value: '' , showItems: 3 , subjectlength : 0 , snackbar: false ,  Transition: null, rankindex: 0 , rankdirection: "next" ,  rankitems: [], hasMore: false,loadmoredownbuttononallrank: false,
            ...
            return(
               ...
               {//this.props.data.subject_ranking && console.log(this.props.data.subject_ranking , "----------------------")
                               ...
                               Test Rank
                                ...
                                <Grid item xs={6} sm={3} lg={2} md={3}  style={{textAlign: "left"}}   align="left" style={{backgroundColor: "#383e49"}}>
                                <MainDashboardTestRankSubject value={this.state.value} handleChangeOption={this.handleChangeOption} subjects={this.props.subjects} fetched={this.props.subjectfetched} onclicksubject={this.onclicksubject}/> 
                                </Grid>
                                <Grid item xs={6} sm={3} lg={2} md={3}  style={{textAlign: "left"}}   align="left" style={{backgroundColor: "#383e49"}}>
                                <Button className="btn btn-info" style={{border:"solid white 2px", color:"white" , margin: 10 , backgroundColor: "rgb(56, 62, 73)" , fontWeight: 600}} onClick={() =>{this.handleshowall()}}>See All</Button>
                                </Grid>
                              {this.props.subjects.subjects && this.state.subject==="" ? this.setState({subject :this.props.subjects.subjects[0][0] }) : "" }
                                <Grid item xs={12} sm={12} lg={12} md={12}  style={{textAlign: "center"}} className="subjecttext" justify="center" align="center" style={{backgroundColor: "white"}}>
                           ...
                           <table className="ranktable table-hover mybox table-hover" style={{backgroundColor: "white" , }}>
                              ...
                              {// this is for up load more buttton
                                ...
                                <td colspan="5" style={{textAlign: "center"}} >{this.props.data.subject_ranking && this.props.data.subject_ranking.length > 3 &&this.state.loadmoredownbuttononallrank ? <button className="loadmorebtn" onClick={()=>this.onloadmoreranksup(rankstartindex)}>Load more</button>: ""}</td>
                                    ...
                                    <tr className={localStorage.getItem('username')===val.username ? "found" : "oftable"} key={i} /*ref={(element) => this._nodes.set(i, element)}*/  /*onScroll={}*/ ref="myscroll" style={{backgroundColor: ""}} onClick={()=>this.showprofile(val.username )} >
...
const mapStateToProps = state => {
  ...
  export default connect(mapStateToProps , { fetchMainDashboardTestRank , fetchMainDashboardTestRankProfile , pagination_rank_onclickall,fetching_maindashboard_loadmore , fetching_maindashboard_loadmoreup})(MainDashboardTestRank);